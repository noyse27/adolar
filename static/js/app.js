...
const radio = { active: false, browsingLibrary: false, queue: [], history: [], playedIds: [], shuffleSession: null, cfTimer: null, cfActive: false, cfPending: false, cfToken: 0 };
radio.stationId = 1;
...
function updatePlayView(track) {
  ...
  const sourceIndex = state.tracks.findIndex(item => String(item.id) === String(track.id));
  $("play-view-prev").disabled = !state.currentTrack;
...
function startNormalCrossfade() {
  if (radio.active || !normalCrossfade.enabled || normalCrossfade.active || normalCrossfade.pending) return;
  ...
  const beginFade = () => {
    ...
    const tick = () => {
      if (!normalCrossfade.active || token !== normalCrossfade.token) return;
...
function startCrossfade() {
  ...
  const beginFade = () => {
    ...
    const startTime = performance.now();
    const tick = () => {
      if (!radio.cfActive || token !== radio.cfToken) return;
      const elapsed = (performance.now() - startTime) / 1000;
...
audio.onended = async () => {
  ...
  if (radio.active) {
    ...
    if (radio.cfPending) resetRadioCrossfadeBuffer();
    await radioNext();
    ...
    if (normalCrossfade.pending) resetNormalCrossfadeBuffer();
    if (listShuffle.active) {
...
audio.ontimeupdate = () => {
  ...
  if (!_playCounted && audio.currentTime / audio.duration >= 0.9) {
    ...
    if (state.currentTrack) {
      fetch(`${API}/api/track/${state.currentTrack.id}/played`, {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({source: adolar4uSource()}),
      })
        .catch(() => {});
    }
  }
...
$("btn-prev").onclick = () => {
  ...
  if (radio.active) {
    const previousTrack = radio.history.pop();
    if (!previousTrack) {
      audio.currentTime = 0;
      return;
    }
    resetRadioCrossfadeBuffer();
    const lastPlayedId = radio.playedIds[radio.playedIds.length - 1];
    if (String(lastPlayedId) === String(previousTrack.id)) radio.playedIds.pop();
    radio.tracksSinceJingle = Math.max(0, radio.tracksSinceJingle - 1);
    radio.queue.unshift(previousTrack);
    state.tracks = [...radio.queue];
    state.currentIdx = 0;
